...
public class ContributionsController {
    ...
    public ContributionsController(ContributionOfferRepository contributionOfferRepository,
                                   ...
                                   AccountRepository accountRepository, TransferService transferService, TransferValidationService transferValidationService, ContributionService contributionService, CurrencyConvertService currencyConvertService) {
    ...
    public String contributions(Principal principal, Model model, @ModelAttribute("contribution") Contribution contribution1) {
        ...
        List<Account> accounts = client.getAccounts().stream().filter(p -> p.getType().equals(AccountType.CHECKING_ACCOUNT)).toList();
